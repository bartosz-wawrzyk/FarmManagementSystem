...
public class IncomeController {
    ...
    @FXML
    private TableColumn<Income, String> col_classification;

    @FXML
    private TableColumn<Income, Float> col_count;

    @FXML
    private TableColumn<Income, Date> col_date;

    @FXML
    private TableColumn<Income, Integer> col_id;

    @FXML
    private TableColumn<Income, String> col_name;

    @FXML
    private TableColumn<Income, String> col_type;

    @FXML
    private TableColumn<Income, Float> col_value;

    @FXML
    private ComboBox<Income> combobox_category;

    @FXML
    private ComboBox<Income> combobox_type;

    @FXML
    private DatePicker date_expiration;

    @FXML
    private TableView<Income> tableviewIncome;
    ...
    void delete() throws SQLException{
        String sql = "DELETE FROM przychody WHERE id = '" + txt_id.getText() + "'";
    ...
    void insert() {
        ...
        String sql = "INSERT INTO przychody VALUES(?, ?, ?, ?, ?, ?, ?)";
    ...
    @FXML
    void select() {
        int num = tableviewIncome.getSelectionModel().getSelectedIndex();

        Income income = tableviewIncome.getSelectionModel().getSelectedItem();

        if(num-1 < -1)
            return;

        txt_id.setText(String.valueOf(income.getId()));
        txt_value.setText(String.valueOf(income.getValue()));
        String txt_date_review = String.valueOf(date_expiration.getValue());
        date_expiration.setAccessibleHelp(txt_date_review);
        txt_name.setText(income.getName());
        txt_count.setText(String.valueOf(income.getAmount()));
        combobox_type.getSelectionModel().clearSelection();
        combobox_category.getSelectionModel().clearSelection();
    }
    ...
    void update() {
        ...
        String sql = "UPDATE przychody SET id = '" + txt_id.getText()
    ...
    public ObservableList<Income> listData(){
        ObservableList<Income> dataList = FXCollections.observableArrayList();

        connection = database.connectDb();

        String sql = "SELECT * FROM przychody";
        ...
        try{
            ...
            while(resultSet.next()){
                income = new Income(resultSet.getInt("id"),
    ...
    public void showData(){
        ObservableList<Income> show = listData();
